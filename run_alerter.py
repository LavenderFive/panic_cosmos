...
from src.alerting.alerts.alerts import (
    TerminatedDueToExceptionAlert,
    NodeInaccessibleDuringStartup,
    RepoInaccessibleDuringStartup,
)
...
from src.monitoring.monitors.monitor_starters import (
    start_node_monitor,
...
from src.utils.config_parsers.internal_parsed import (
    InternalConf,
    INTERNAL_CONFIG_FILE_FOUND,
    INTERNAL_CONFIG_FILE,
)
from src.utils.config_parsers.user import NodeConfig, RepoConfig
from src.utils.config_parsers.user_parsed import UserConf, MISSING_USER_CONFIG_FILES
...
def node_from_node_config(node_config: NodeConfig):
    # Test connection
    log_and_print("Trying to connect to {}/status" "".format(node_config.node_rpc_url))
    try:
        node_status = get_cosmos_json(
            node_config.node_rpc_url + "/status", logger_general
        )
        log_and_print("Success.")
    except Exception:
        raise InitialisationException(
            "Failed to connect to {} at {}".format(
...
def test_connection_to_github_page(repo: RepoConfig):
    # Get releases page
    releases_page = InternalConf.github_releases_template.format(repo.repo_page)
    ...
    try:
        ...
        if "message" in releases and releases["message"] == "Not Found":
            raise InitialisationException(
                "Successfully reached {} but URL " "is not valid.".format(releases_page)
            )
        else:
            log_and_print("Success.")
    except Exception:
        raise InitialisationException("Could not reach {}." "".format(releases_page))
...
def run_monitor_nodes(node: Node):
    ...
    node_monitor = NodeMonitor(
        monitor_name, full_channel_set, logger_monitor_node, REDIS, node
    )
    ...
    while True:
        ...
        try:
            ...
            full_channel_set.alert_error(TerminatedDueToExceptionAlert(monitor_name, e))
...
def run_monitor_network(network_nodes_tuple: Tuple[str, List[Node]]):
    ...
    try:
        ...
        logger_monitor_network = create_logger(
            InternalConf.network_monitor_general_log_file_template.format(network),
            network,
            InternalConf.logging_level,
            rotating=True,
        )
        ...
        if 0 in [len(validators), len(full_nodes)]:
            log_and_print(
                "!!! Could not start {}. It must have at least 1 "
        ...
        network_monitor = NetworkMonitor(
            ...
            InternalConf.network_monitor_max_catch_up_blocks,
            REDIS,
            full_nodes,
            validators,
        )
    ...
    while True:
        # Start
        log_and_print(
            "{} started with {} validator(s) and {} full node(s)."
...
if __name__ == "__main__":
    if not INTERNAL_CONFIG_FILE_FOUND:
        ...
        sys.exit(
            "Config file {} is missing. Make sure that you run the setup "
    ...
    log_and_print(
        "Enabled alerting channels (general): {}".format(
            full_channel_set.enabled_channels_list()
        )
    )
    periodic_alive_reminder_channel_set = get_periodic_alive_reminder_channel_set(
        alerter_name, logger_general, REDIS, log_file_alerts
    )
    log_and_print(
    ...
    for n in UserConf.filtered_nodes:
        try:
            ...
            if n.node_is_validator:
                full_channel_set.alert_major(NodeInaccessibleDuringStartup(n.node_name))
            else:
                full_channel_set.alert_minor(NodeInaccessibleDuringStartup(n.node_name))
    ...
    nodes_by_network = {
        net: [node for node in network_monitor_nodes if node.network == net]
    ...
    for r in UserConf.filtered_repos:
        try:
            ...
            full_channel_set.alert_minor(RepoInaccessibleDuringStartup(r.repo_name))
    ...
    total_count = sum(
        [
            monitor_node_count,
            monitor_network_count,
            monitor_github_count,
            commands_telegram_count,
