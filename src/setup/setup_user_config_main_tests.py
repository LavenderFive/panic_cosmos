...
def test_email_alerts(
    email_smtp: str, email_from: str, email_to: str, email_user: str, email_pass: str
) -> TestOutcome:
...
def test_twilio_alerts(
    ...
    try:
        twilio_api.dial_number(
            twilio_no, to_dial, internal_conf.twiml, internal_conf.twiml_is_url
        )
...
def test_redis(host: str, port: str, password: str) -> TestOutcome:
    redis = RedisApi(DUMMY_LOGGER, 0, host, int(port), password=password, namespace="")
    try:
        redis.ping_unsafe()
