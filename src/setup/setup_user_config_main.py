...
from src.setup.setup_user_config_main_tests import (
...
def setup_general(cp: ConfigParser) -> None:
    print("==== General")
    print(
        "The first step is to set a unique identifier for the alerter. This "
...
def setup_telegram_alerts(cp: ConfigParser) -> None:
    ...
    if is_already_set_up(cp, "telegram_alerts") and not yn_prompt(
        "Telegram alerts are already set up. Do you "
...
def setup_email_alerts(cp: ConfigParser) -> None:
    ...
    if is_already_set_up(cp, "email_alerts") and not yn_prompt(
        "Email alerts are already set up. Do you "
        "wish to clear the current config? (Y/n)\n"
    ):
        return
    ...
    while True:
        ...
        email_user = input(
            "Please insert the username for SMTP authentication "
            "(blank for no authentication):\n"
        )
        if len(email_user) != 0:
            email_pass = input(
                "Please insert the password for SMTP " "authentication:\n"
            )
        else:
            email_pass = ""

        email_from = input(
            "Please specify the details of the sender in the "
            "format shown below:\n\t"
            "example_sender@email.com\n"
        )

        email_to = input(
            "Please specify the email address where you wish to "
        ...
        if yn_prompt(
            "Do you wish to test email alerts now? The first "
            "email address inserted will be used. (Y/n)\n"
        ):
            test = test_email_alerts(
                email_smtp, email_from, email_to.split(";")[0], email_user, email_pass
            )
            if test == TestOutcome.RestartSetup:
                continue
            elif test == TestOutcome.SkipSetup:
...
def setup_twilio_alerts(cp: ConfigParser) -> None:
    ...
    if is_already_set_up(cp, "twilio_alerts") and not yn_prompt(
        "Twilio alerts are already set up. Do you "
        "wish to clear the current config? (Y/n)\n"
    ):
        return
    ...
    while True:
        ...
        twilio_no = input(
            "Please insert your registered Twilio phone number "
            "in the format shown below:\n\t"
            "E.164 format, for example: +12025551234\n"
        )

        to_dial = input(
            "Please insert the first phone number to dial for "
        ...
        if yn_prompt(
            "Do you wish to test Twilio alerts now? The first "
            "phone number inserted will be called. (Y/n)\n"
        ):
            test = test_twilio_alerts(twilio_no, to_dial.split(";")[0], twilio_api)
            if test == TestOutcome.RestartSetup:
                continue
            elif test == TestOutcome.SkipSetup:
                return
...
def setup_alert_channels(cp: ConfigParser) -> None:
    print("==== Alerts")
    print(
        "By default, alerts are output to a log file and to "
...
def setup_telegram_commands(cp: ConfigParser) -> None:
    ...
    if is_already_set_up(cp, "telegram_commands") and not yn_prompt(
        "Telegram commands are already set up. Do you "
        "wish to clear the current config? (Y/n)\n"
    ):
        return

    print(
        "NOTE: If you are running more than one instance of the PANIC "
...
def setup_redis(cp: ConfigParser) -> None:
    ...
    if is_already_set_up(cp, "redis") and not yn_prompt(
        "Redis is already set up. Do you wish " "to clear the current config? (Y/n)\n"
    ):
        return
...
def setup_periodic_alive_reminder(cp: ConfigParser) -> None:
    ...
    if is_already_set_up(cp, "periodic_alive_reminder") and not yn_prompt(
        "The periodic alive reminder is already set up. "
    ...
    while True:
        try:
            ...
            interval = input(
                "Input is not a valid integer. Please enter " "another value:\n"
            )
            continue
        if interval_number_rep > 0:
            ...
            interval = input(
                "Input is not a positive integer. Please enter " "another value:\n"
            )

    if (
        is_already_set_up(cp, "email_alerts")
        and cp["email_alerts"]["enabled"]
        and yn_prompt(
            "Would you like the periodic alive reminder "
    ...
    if (
        is_already_set_up(cp, "telegram_alerts")
        and cp["telegram_alerts"]["enabled"]
        and yn_prompt(
            "Would you like the periodic alive reminder "
