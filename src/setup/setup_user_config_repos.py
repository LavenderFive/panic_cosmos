...
def get_repo(repos_so_far: List[RepoConfig]) -> Optional[RepoConfig]:
    ...
    while True:
        repo_page = input(
            "Official GitHub repository (example: " "cosmos/cosmos-sdk/):\n"
        )
        ...
        try:
            ...
            if not yn_prompt(
                "Failed to connect to page. Do " "you want to try again? (Y/n)\n"
            ):
                return None
...
def setup_repos(cp: ConfigParser) -> None:
    ...
    if len(cp.sections()) > 0 and not yn_prompt(
        "The list of repositories is already set up. Do you "
