...
def get_node(nodes_so_far: List[NodeConfig]) -> Optional[NodeConfig]:
    ...
    while True:
        ...
        try:
            ...
            if not yn_prompt(
                "Failed to connect to endpoint. Do " "you want to try again? (Y/n)\n"
            ):
                return None
...
def setup_nodes(cp: ConfigParser) -> None:
    ...
    if len(cp.sections()) > 0 and not yn_prompt(
        "The list of nodes is already set up. Do you wish to "
    ...
    for i, node in enumerate(nodes):
        ...
        cp[section]["node_is_validator"] = "true" if node.node_is_validator else "false"
