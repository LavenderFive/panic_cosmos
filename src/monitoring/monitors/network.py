...
class NetworkMonitor(Monitor):
    def __init__(
        ...
        self.network_monitor_max_catch_up_blocks = network_monitor_max_catch_up_blocks
    ...
    def _check_block(self, height: int) -> None:
        self._logger.info("%s obtaining data at height %s", self._monitor_name, height)
        ...
        if "precommits" in last_commit:
            ...
            non_null_precommits = filter(lambda p: p["signature"], block_precommits)
        block_precommits_validators = set(
            map(lambda p: p["validator_address"], non_null_precommits)
        )
        total_no_of_missing_validators = len(block_precommits) - len(
            block_precommits_validators
        )

        self._logger.debug("Precommit validators: %s", block_precommits_validators)
        self._logger.debug(
            "Total missing validators: %s", total_no_of_missing_validators
        )
    ...
    def monitor(self) -> None:
        ...
        if (
            last_height_to_check - self._last_height_checked
            > self.network_monitor_max_catch_up_blocks
        ):
            height = last_height_to_check - self.network_monitor_max_catch_up_blocks
