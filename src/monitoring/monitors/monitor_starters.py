...
def start_node_monitor(
    node_monitor: NodeMonitor, monitor_period: int, logger: logging.Logger
):
    # Start
    while True:
        # Read node data
...
def start_network_monitor(
    network_monitor: NetworkMonitor, monitor_period: int, logger: logging.Logger
):
    # Start
    while True:
        # Read network data
        try:
            ...
            network_monitor.channels.alert_error(
                ErrorWhenReadingDataFromNode(network_monitor.last_full_node_used)
            )
            logger.error(
                "Error when reading data from %s: %s",
