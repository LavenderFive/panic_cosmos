...
class GitHubMonitor(Monitor):
    def __init__(
        ...
        self._redis_github_releases_key = redis_github_releases_key_prefix + repo_name
    ...
    def save_state(self) -> None:
        ...
        if self.redis_enabled:
            if self._prev_no_of_releases is None:
                self.logger.debug(
                    "Skipping github monitor state save "
            ...
            self.redis.set(self._redis_github_releases_key, self._prev_no_of_releases)
    ...
    def monitor(self) -> None:
        ...
        if self._prev_no_of_releases not in [None, no_of_releases]:
            if self._prev_no_of_releases < no_of_releases:
                ...
                self.logger.error(
                    "The number of releases decreased from {} "
                    "to {}: {}".format(
                        self._prev_no_of_releases, no_of_releases, releases
                    )
                )
                self._prev_no_of_releases = None
