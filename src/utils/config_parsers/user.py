...
class NodeConfig:
    def __init__(
        ...
        include_in_network_monitor: bool,
    ) -> None:
...
class RepoConfig:
    def __init__(
        self, repo_name: str, repo_page: str, include_in_github_monitor: bool
    ) -> None:
...
class UserConfig(ConfigParser):
    ...
    def __init__(
        self,
        alerting_config_file_path: str,
        nodes_config_file_path: str,
        repos_config_file_path: str,
    ) -> None:
        super().__init__(
            [alerting_config_file_path, nodes_config_file_path, repos_config_file_path]
        )
        ...
        # [general]
        self.unique_alerter_identifier = cp["general"]["unique_alerter_identifier"]
        ...
        self.twilio_dial_numbers = cp["twilio_alerts"]["phone_numbers_to_dial"].split(
            ";"
        )
        ...
        self.interval_seconds = (
            timedelta(seconds=int(cp["periodic_alive_reminder"]["interval_seconds"]))
            if self.periodic_alive_reminder_enabled
            else None
        )
        self.email_enabled = (
            to_bool(cp["periodic_alive_reminder"]["email_enabled"])
            if self.periodic_alive_reminder_enabled
            else None
        )
        self.telegram_enabled = (
            to_bool(cp["periodic_alive_reminder"]["telegram_enabled"])
            if self.periodic_alive_reminder_enabled
            else None
        )
        ...
        self.filtered_nodes = [
            ...
            if n.include_in_node_monitor or n.include_in_network_monitor
        ]
        ...
        self.filtered_repos = [r for r in self.all_repos if r.include_in_github_monitor]
