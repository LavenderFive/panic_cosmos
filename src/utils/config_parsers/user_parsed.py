...
USER_CONFIG_FILES = [
    USER_CONFIG_FILE_MAIN,
    USER_CONFIG_FILE_NODES,
    USER_CONFIG_FILE_REPOS,
]
MISSING_USER_CONFIG_FILES = [f for f in USER_CONFIG_FILES if not os.path.isfile(f)]

if len(MISSING_USER_CONFIG_FILES) == 0:
    UserConf = UserConfig(
        USER_CONFIG_FILE_MAIN, USER_CONFIG_FILE_NODES, USER_CONFIG_FILE_REPOS
    )
else:
    UserConf = None
