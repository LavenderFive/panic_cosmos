...
class InternalConfig(ConfigParser):
    ...
    def __init__(self, config_file_path: str) -> None:
        ...
        self.redis_periodic_alive_reminder_mute_key = section[
            "redis_periodic_alive_reminder_mute_key"
        ]

        self.redis_twilio_snooze_key_default_hours = timedelta(
            hours=float(section["redis_twilio_snooze_key_default_hours"])
        )
        self.redis_periodic_alive_reminder_mute_key_default_hours = timedelta(
            hours=float(
                section["redis_periodic_alive_reminder_mute_key_" "default_hours"]
            )
        )
        ...
        self.node_monitor_period_seconds = int(section["node_monitor_period_seconds"])
        ...
        self.downtime_initial_alert_delay = timedelta(
            seconds=int(section["downtime_initial_alert_delay_seconds"])
        )
        self.downtime_reminder_interval_seconds = timedelta(
            seconds=int(section["downtime_reminder_interval_seconds"])
        )
        self.max_missed_blocks_time_interval = timedelta(
            seconds=int(section["max_missed_blocks_interval_seconds"])
        )
        ...
        self.validator_peer_safe_boundary = int(section["validator_peer_safe_boundary"])
        ...
        self.missed_blocks_info_boundary = int(section["missed_blocks_info_boundary"])
        self.missed_blocks_minor_boundary = int(section["missed_blocks_minor_boundary"])
        self.missed_blocks_danger_boundary = int(
            section["missed_blocks_danger_boundary"]
        )
        self.github_error_interval_seconds = timedelta(
            seconds=int(section["github_error_interval_seconds"])
        )
        ...
        self.block_big_dipper_link_prefix = section["block_big_dipper_link_prefix"]
        self.block_stargazer_link_prefix = section["block_stargazer_link_prefix"]
    ...
    def _peer_safe_and_danger_boundaries_are_valid(self) -> bool:
        return (
            self.validator_peer_safe_boundary > self.validator_peer_danger_boundary > 0
        )

    def _check_if_peer_safe_and_danger_boundaries_are_valid(self):
        while not self._peer_safe_and_danger_boundaries_are_valid():
            print(
                "validator_peer_safe_boundary must be STRICTLY GREATER than "
