...
class TelegramCommands(Commands):
    def __init__(
        ...
        self._default_snooze_hours = internal_conf.redis_twilio_snooze_key_default_hours
    ...
    def _start_callback(self, update: Update, context: CallbackContext):
        ...
        update.message.reply_text(
            "Welcome to the PANIC alerter bot!\n" "Type /help for more information."
        )
    ...
    def _snooze_callback(self, update: Update, context: CallbackContext):
        ...
        if not self._user_conf.twilio_alerts_enabled:
            update.message.reply_text(
                "Snoozing is not available given that " "Twilio calls were not set up."
            )
            return
        ...
        if not self._redis_enabled:
            update.message.reply_text(
                "Snoozing is not available given " "that Redis is not set up."
            )
            return
    ...
    def _unsnooze_callback(self, update: Update, context: CallbackContext):
        ...
        if not self._redis_enabled:
            update.message.reply_text(
                "Unsnoozing is not available given " "that Redis is not set up."
            )
            return
    ...
    def _mute_callback(self, update: Update, context: CallbackContext):
        ...
        if not self._user_conf.periodic_alive_reminder_enabled:
            update.message.reply_text(
                "Muting is not available given that the "
        ...
        if not self._redis_enabled:
            update.message.reply_text(
                "Muting is not available given that " "Redis is not set up."
            )
            return
    ...
    def _unmute_callback(self, update: Update, context: CallbackContext):
        ...
        if not self._redis_enabled:
            update.message.reply_text(
                "Unmuting is not available given " "that Redis is not set up."
            )
            return
    ...
    def _status_callback(self, update: Update, context: CallbackContext):
        ...
        if not self._redis_enabled:
            update.message.reply_text(
                "Status update not available given " "that Redis is not set up."
            )
            return
        ...
        if self._redis.is_live:
            ...
            status += (
                "- Redis is running normally but was not accessible "
        ...
        if self._user_conf.periodic_alive_reminder_enabled:
            if self._redis.exists(self._redis_mute_key):
                ...
                status += (
                    "- The periodic alive reminder has "
                    "been muted until {}.\n".format(until)
                )
            else:
        ...
        net_monitor_names2 = [
            k.replace(self._redis_network_monitor_last_height_key_prefix, "")
    ...
    def _unknown_callback(self, update: Update, context: CallbackContext) -> None:
        self._logger.info(
            "Received unrecognized command: update=%s, context=%s", update, context
        )
