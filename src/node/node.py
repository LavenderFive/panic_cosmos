...
class Node:
    ...
    def load_state(self, logger: logging.Logger) -> None:
        ...
        if self._redis_enabled:
            ...
            if self._went_down_at is not None:
                try:
                    self._went_down_at = dateutil.parser.parse(self._went_down_at)
            ...
            self._redis.set_multiple(
                {
                    ...
                    self._redis_prefix
                    + "_consecutive_blocks_missed": self._consecutive_blocks_missed,
    ...
    def set_as_down(self, channels: ChannelSet, logger: logging.Logger) -> None:
        ...
        logger.debug(
            "%s set_as_down: is_down(currently)=%s, channels=%s",
            self,
            self.is_down,
            channels,
        )
        ...
        # time has passed for it, then send an initial alert
            ...
            if self._downtime_initial_alert_delayer.can_do_task():
                downtime = strfdelta(
                    datetime.now() - self._went_down_at,
                    "{hours}h, {minutes}m, {seconds}s",
                )
                if self.is_validator:
                    channels.alert_major(
                        CannotAccessNodeAlert(self.name, self._went_down_at, downtime)
                    )
                else:
    ...
    def set_as_up(self, channels: ChannelSet, logger: logging.Logger) -> None:
        ...
        logger.debug(
            "%s set_as_up: is_down(currently)=%s, channels=%s",
            self,
            self.is_down,
            channels,
        )
        ...
        if self.is_down:
            ...
            if self._initial_downtime_alert_sent:
                ...
                channels.alert_info(
                    NowAccessibleAlert(self.name, self._went_down_at, downtime)
                )
    ...
    def add_missed_block(
        ...
        logger.debug(
            ...
            "channels=%s",
            self,
            self.consecutive_blocks_missed_so_far,
            blocks_missed,
            missing_validators,
            channels,
        )
    ...
    def clear_missed_blocks(self, channels: ChannelSet, logger: logging.Logger) -> None:
        # NOTE: This function assumes that the node is a validator
    ...
    def set_voting_power(
        self, new_voting_power: int, channels: ChannelSet, logger: logging.Logger
    ) -> None:
        # NOTE: This function assumes that the node is a validator
    ...
    def set_catching_up(
        self, now_catching_up: bool, channels: ChannelSet, logger: logging.Logger
    ) -> None:
    ...
    def set_no_of_peers(
        self, new_no_of_peers: int, channels: ChannelSet, logger: logging.Logger
    ) -> None:
            ...
            if self.is_validator:
                ...
                if new_no_of_peers > self.no_of_peers:  # increase
                    if new_no_of_peers <= danger:
                        ...
                        channels.alert_info(
                            PeersIncreasedOutsideDangerRangeAlert(self.name, danger)
                        )
