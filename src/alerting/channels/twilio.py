...
class TwilioChannel(Channel):
    ...
    def _calls_snoozed(self, logger: logging.Logger) -> bool:
        if self.redis_enabled:
            if self.redis.exists(self._snooze_key):
                ...
                logger.info(
                    "Tried to call but calls are snoozed until {}."
                    "".format(snooze_until)
                )
                return True
            else:
    ...
    def alert_major(self, alert: Alert) -> None:
        ...
        try:
            ...
            self.logger.error("Error when checking if Twilio calls are snoozed: %s.", e)
            snoozed = False
            ...
            for number in self._call_to:
                ...
                try:
                    self._twilio.dial_number(
                        self._call_from, number, self._twiml, self._twiml_is_url
                    )
                except Exception as e:
                    self._backup_channels.alert_error(
