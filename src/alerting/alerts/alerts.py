...
class ExperiencingDelaysAlert(Alert):
    def __init__(self, node: str) -> None:
        super().__init__("Experiencing delays when trying to access {}.".format(node))


class CannotAccessNodeAlert(Alert):
    def __init__(self, node: str, went_down_at: datetime, downtime: str) -> None:
        super().__init__(
...
class CouldNotFindLiveFullNodeAlert(Alert):
    def __init__(self, network_monitor: str) -> None:
        super().__init__(
            "{} could not find a live full node to use as a "
...
class MissedBlocksAlert(Alert):
    def __init__(
        self, node: str, blocks: int, height: int, missing_validators: int
    ) -> None:
        super().__init__(
...
class PeersIncreasedAlert(Alert):
    def __init__(self, node: str, old_peers: int, new_peers: int) -> None:
        super().__init__(
            "{} peers INCREASED from {} to {}.".format(node, old_peers, new_peers)
        )
...
class PeersDecreasedAlert(Alert):
    def __init__(self, node: str, old_peers: int, new_peers: int) -> None:
        super().__init__(
            "{} peers DECREASED from {} to {}.".format(node, old_peers, new_peers)
        )
...
class ProblemWhenCheckingIfCallsAreSnoozedAlert(Alert):
    def __init__(self) -> None:
        super().__init__(
            "Problem encountered when checking if "
